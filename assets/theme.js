...
theme.Product = (function() {
  function Product(container) {
    ...
    if (this.settings.zoomEnabled) {
      this.settings.imageZoomSize = theme.Images.imageSize(
        $(this.selectors.productImageWrap).data('zoom')
      )
    }
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _initVariants: function() {
      ...
      theme.variant = this.variants

      // Swatches
      if (this.variants.currentVariant) {
        var form = $('form[action="/cart/add"]')
        for (
          var i = 0, length = this.variants.currentVariant.options.length;
          i < length;
          i++
        ) {
          var radioButton = form.find(
            '.swatch[data-option-index="' +
              i +
              '"] :radio[value="' +
              this.variants.currentVariant.options[i] +
              '"]'
          )
          if (radioButton.size()) {
            radioButton.get(0).checked = true
            var group = radioButton.attr('data-group')

            radioButton
              .closest('.swatch')
              .find('.js-swatch-result[data-group=' + group + ']')
              .html(' - ' + this.variants.currentVariant.options[i])

            // Swatch label
            var id = this.variants.currentVariant.id
            var metafields = totallee.metafieldsById[id] || {}
            var label = metafields.swatch_label || ''
            var displayMethod = label ? 'addClass' : 'removeClass'

            radioButton
              .closest('.swatch')
              .find('.js-swatch-label[data-group=' + group + ']')
              .html(label)[displayMethod]('visible')
          }
        }
      }
    },
    ...
    _updateAddToCart: function(evt) {
      ...
      // theme.variant.currentVariant = variant
